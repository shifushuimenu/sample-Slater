...
header = '# Sample diag. elements of Greens function independently: \
max_HS_samples = %d, Nsamples_per_HS = %d' \
    % (max_HS_samples, Nsamples_per_HS)
header_labels = header + "\n" + column_labelling
np.savetxt('prob_Fock_ncpu%5.5d_sample_diag.dat' % (MPI_rank), MM, header=header_labels)
...
if MPI_rank == 0:
    ...
    header_labels = header + "\n" + column_labelling
    np.savetxt('prob_Fock_allCPUs_sample_diag.dat', MM_allCPUs, header=header_labels)
